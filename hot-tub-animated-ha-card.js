...
class HotTubCard extends HTMLElement{
 ...
 static getConfigForm(){return{schema:[
  ...
  {name:'pump_entity',required:true,selector:{entity:{filter:{domain:'fan'}}}},
  {type:'expandable',name:'activities',title:'Live activities',flatten:true,schema:[
   {name:'temperature_entity',selector:{entity:{filter:{domain:'sensor'}}}},
   {name:'target_temperature_entity',selector:{entity:{filter:{domain:'sensor'}}}},
   {name:'heating_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'ozone_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'circulating_pump_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'spa_in_use_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'filter_clean_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'filter_purge_entity',selector:{entity:{filter:{domain:'binary_sensor'}}}},
   {name:'light_entity',selector:{entity:{filter:{domain:'light'}}}},
   {name:'status_entity',selector:{entity:{filter:{domain:'sensor'}}}}
  ]},
  {type:'expandable',name:'display',title:'Display',flatten:true,schema:[
   {name:'show_activities',selector:{boolean:{}}},
   {name:'show_temperature',selector:{boolean:{}}}
  ]}
 ],computeLabel:s=>({title:'Card title',pump_entity:'Pump 1',temperature_entity:'Current temperature',target_temperature_entity:'Target temperature',heating_entity:'Heating',ozone_entity:'Ozone',circulating_pump_entity:'Circulating pump',spa_in_use_entity:'Spa in use',filter_clean_entity:'Filter clean',filter_purge_entity:'Filter purge',light_entity:'Spa lights',status_entity:'Status',show_activities:'Show live activities',show_temperature:'Show temperature'})[s.name],computeHelper:s=>s.name==='pump_entity'?'Pump 1 preset OFF / LO / HI controls the animation.':undefined,assertConfig:config=>{if(config?.pump_entity&&typeof config.pump_entity!=='string')throw new Error('Pump 1 must be an entity ID.')}}}
 ...
 active(id){const s=String(this.stateObj(id)?.state??'').toLowerCase();return['on','active','heating','clean','purge','yes','connected'].includes(s)}
